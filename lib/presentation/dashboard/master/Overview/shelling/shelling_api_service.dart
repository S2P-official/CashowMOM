...
class ShellingApiService {
  static const String baseUrl = 'http://192.168.29.215:8080/api';

  Future<ShellingSummaryResponse> fetchTodaySummary(int tenantId) async {
    final today = DateFormat('yyyy-MM-dd').format(DateTime.now());

    final url =
        '$baseUrl/shelling-reports/tenant/$tenantId/date/$today';

    final response = await http.get(Uri.parse(url));

    if (response.statusCode == 200) {
      return ShellingSummaryResponse.fromJson(
