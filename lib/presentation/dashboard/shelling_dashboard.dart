...
// ---------------- Dashboard App ---------------- //
class ShellingDashboardApp extends StatelessWidget {
  final String tenantName;
  final int tenantId;
  final int employeeId;

  const ShellingDashboardApp({
    super.key,
...
class _TasksPageState extends State<TasksPage> {
  ...
  Future<void> fetchReports() async {
    final apiUrl =
        "http://192.168.29.215:8080/api/roasting-reports/tenant/$tenantId/pending";
  ...
  void _openUpdateDialog(Map<String, dynamic> report) {
    final wholesCtrl =
        TextEditingController(text: report["wholes"]?.toString() ?? "");
    final brokenCtrl =
        TextEditingController(text: report["broken"]?.toString() ?? "");
    final rejectionCtrl =
        TextEditingController(text: report["rejection"]?.toString() ?? "");
    final uncutCtrl =
        TextEditingController(text: report["uncut"]?.toString() ?? "");
    final partlyCtrl =
        TextEditingController(text: report["partly"]?.toString() ?? "");
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _readOnlyField(
                    "Total Production Mts", report["totalProductionMts"]),
                ...
                _readOnlyField("Roaster Name", report["roasterName"]),
                _readOnlyField(
                    "Temp For VN Machine", report["tempForVnMachine"]),
                _readOnlyField(
                    "Roasting Duration", report["roastingDuration"]),
                _readOnlyField("Soacking Moisture", report["soackingMoisture"]),
                _readOnlyField(
                    "Moisture After Roasting", report["moistureAfterRoasting"]),
                _readOnlyField("Cooking Time", report["cookingTime"]),
                _readOnlyField("Dry RCN Moisture", report["dryRcnMoisture"]),
                _readOnlyField("Total Roasted", report["totalRoasted"]),
                _readOnlyField("Cutting Line", report["cuttingLine"]),

                const SizedBox(height: 12),

                // Editable Fields
                textBox("Wholes", wholesCtrl),
                textBox("Broken", brokenCtrl),
                textBox("Rejection", rejectionCtrl),
                textBox("Uncut", uncutCtrl),
                textBox("Partly", partlyCtrl),
              ],
            ),
          ),
          actions: [
  ...
  Future<void> submitRoastingReport(
    ...
    final postUrl = Uri.parse(
        "http://192.168.29.215:8080/api/shelling-reports/tenant/$tenantId/employee/$employeeId");

    final patchUrl = Uri.parse(
        "http://192.168.29.215:8080/api/roasting-reports/${report["id"]}");
    ...
    try {
      ...
      if (postRes.statusCode == 200 || postRes.statusCode == 201) {
        ...
        if (patchRes.statusCode == 200) {
          setState(() => report["status"] = "Completed");
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text("Total Roasted: ${r["totalRoasted"]}"),
...
class _ViewTasksPageState extends State<ViewTasksPage> {
  ...
  Future<void> fetchCookingReports() async {
    final url = Uri.parse(
      'http://192.168.29.215:8080/api/shelling-reports/tenant/${widget.tenantId}/employee/${widget.employeeId}',
    );
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (isExpanded) ...[
                            ...
                            _buildInfoRow(
                              "Temp For VN Machine",
                              task["tempForVnMachine"],
                            ),
                            _buildInfoRow(
                              "Roasting Duration",
                              task["roastingDuration"],
                            ),
                            _buildInfoRow(
                              "Soacking Moisture",
                              task["soackingMoisture"],
                            ),
                            _buildInfoRow(
                              "Moisture After Roasting",
                              task["moistureAfterRoasting"],
                            ),
                            _buildInfoRow(
                              "Total Roasted",
                              task["totalRoasted"],
