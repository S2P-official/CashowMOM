...
// ---------------- Dashboard App ---------------- //
class PeelingDashboardApp extends StatelessWidget {
  final String tenantName;
  final int tenantId;
  final int employeeId;

  const PeelingDashboardApp({
    super.key,
...
class _TasksPageState extends State<TasksPage> {
  ...
  void _openUpdateDialog(Map<String, dynamic> report) {
    // Wholes – process stage
    final wwPeeledWholesCtrl = TextEditingController();
    final addTestaWholesCtrl = TextEditingController();
    final unpeeledWholesCtrl = TextEditingController();

    // Brokens / other outputs
    final unpeeledBrokensCtrl = TextEditingController();
    final sauPlCtrl = TextEditingController();
    final rejectionCtrl = TextEditingController();
    final shellBrokensCtrl = TextEditingController();
    final huskCtrl = TextEditingController();


    // Totals (should be read-only / auto-calculated)
    final totalWholesCtrl = TextEditingController();
    final totalBrokenCtrl = TextEditingController();
    final totalRejectionCtrl = TextEditingController();
    ...
    showDialog(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          title: const Text("Update Peeling Result"),
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _textBox("WW Peeled Wholes", wwPeeledWholesCtrl),
                _textBox("Add Testa Wholes", addTestaWholesCtrl),
                _textBox("Unpeeled Wholes", unpeeledWholesCtrl),
                _textBox("Unpeeled Brokens", unpeeledBrokensCtrl),
                _textBox("SAU (PL)", sauPlCtrl),
                _textBox("Rejection", rejectionCtrl),
                _textBox("Shell Brokens", shellBrokensCtrl),
                _textBox("Husk", huskCtrl),

                _textBox("Total Wholes", totalWholesCtrl),
                _textBox("Total Broken", totalBrokenCtrl),
                _textBox("Total Rejection", totalRejectionCtrl),
                _textBox("Total", totalCtrl),
                _textBox("Grand Total", grandTotalCtrl),
              ],
            ),
          ),
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                final success = await submitpeelingReport(
                  report,
                  wwPeeledWholesCtrl.text,
                  addTestaWholesCtrl.text,
                  unpeeledWholesCtrl.text,
                  unpeeledBrokensCtrl.text,
                  sauPlCtrl.text,
                  rejectionCtrl.text,
                  shellBrokensCtrl.text,
                  huskCtrl.text,
                  totalWholesCtrl.text,
                  totalBrokenCtrl.text,
                  totalRejectionCtrl.text,
                  totalCtrl.text,
                  grandTotalCtrl.text,
                );
  ...
  Widget _jsonReadOnlyFields(Map<String, dynamic> report) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: report.entries.where((e) => !excludedKeys.contains(e.key)).map((
        e,
      ) {
        return _readOnlyField(_formatLabel(e.key), e.value);
      }).toList(),
    );
  }
  ...
  // ----------------------------------------------------------------------
  Future<bool> submitpeelingReport(
    Map<String, dynamic> report,
    String WW_Peeled_Wholes,
    String Add_Testa_Wholes,
    String Unpeeled_Wholes,
    String Unpeeled_Brokens,
    String SAU_pl,
    String Rejection,
    String Sh_Brokens,
    String Husk,
    ...
    final payload = {
      "WW_Peeled_Wholes": num(WW_Peeled_Wholes),
      "Add_Testa_Wholes": num(Add_Testa_Wholes),
      "Unpeeled_Wholes": num(Unpeeled_Wholes),
      "Unpeeled_Brokens": num(Unpeeled_Brokens),
      "SAU_pl": num(SAU_pl),
      "Rejection": num(Rejection),
      "Sh_Brokens": num(Sh_Brokens),
      "Husk": num(Husk),
      "Total_Wholes": num(Total_Wholes),
      "Total_Broken": num(Total_Broken),
      "Total_Rejection": num(Total_Rejection),
      "Total": num(Total),
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Submission failed (${postRes.statusCode})")),
      );
    } catch (e) {
...
class _ViewTasksPageState extends State<ViewTasksPage> {
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          _summaryRow("% Final", totalPercent, isPercent: true),
                          _summaryRow(
                            "% Short",
                            totalShortPercent,
                            isPercent: true,
                        ...
                        rows: reports.map((r) {
                          return DataRow(
                            cells: [
                              ...
                              DataCell(
                                Text(
                                  _toDouble(r['totalFinal']).toStringAsFixed(2),
                                ),
                              ),
                              DataCell(
                                Text(
                                  _toDouble(r['totalShort']).toStringAsFixed(2),
                                ),
                              ),
                              DataCell(
                                Text(
  ...
  // ---------------- HELPERS ----------------
  Widget _summaryRow(String label, double value, {bool isPercent = false}) {
    return Padding(
